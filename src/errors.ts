...
export function makeError(args: MakeErrorArgs): TrueSpeechError {
  const error: TrueSpeechError = {
    code: args.code,
    message: args.message,
    span: args.span,
  };
  if (args.notes && args.notes.length > 0) error.notes = args.notes;
  if (args.help !== undefined) error.help = args.help;
...
function summarize(errors: TrueSpeechError[]): string {
...
//
export function renderError(error: TrueSpeechError, source: string): string {
