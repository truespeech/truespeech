...
function validateCompute(
  ...
  if (stmt.metrics.length === 0) return;
  ...
  if (stmt.metrics.length > 1) {
    ...
    errors.push(
      makeError({
        ...
        message:
          "Multi-metric COMPUTE is not supported in this version",
...
function validateRegister(
  ...
  for (const clause of stmt.impactClauses) {
    if (clause.metrics.length === 0) continue;
      ...
      if (!primaryTime) {
        errors.push(
          makeError({
            ...
            span: metricRef.span,
            help:
              "Mark a time field on the metric's dataset as is_primary: true",
          })
        );
        continue;
      }
...
function validateRegisterBoundary(
  stmt: RegisterStatement & { entryKind: "boundary" },
  adapter: SemanticLayerAdapter,
  errors: TrueSpeechError[]
): void {
  ...
  if (!primaryTime) {
    errors.push(
      makeError({
        code: "missing_primary_time",
        message:
          "This metric has no primary time dimension declared in the semantic model",
        span: region.span,
        help:
          "Mark a time field on the metric's dataset as is_primary: true",
      })
...
export function resultColumnNames(stmt: ComputeStatement): string[] {
  const cols: string[] = [];
  if (stmt.groupBy) {
    for (const g of stmt.groupBy) {
      if (g.kind === "bare-grain") cols.push(g.grain);
      else if (g.kind === "dimension") cols.push(g.dimension.name);
...
function compareLiteral(a: TimeLiteral, b: TimeLiteral): number {
  if (a.year !== b.year) return a.year - b.year;
  if (a.unit === "quarter") return (a.quarter ?? 0) - (b.quarter ?? 0);
  if (a.unit === "month") return (a.month ?? 0) - (b.month ?? 0);
  if (a.unit === "day") {
