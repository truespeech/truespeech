...
class Parser {
  ...
  parseRegisterBoundaryTail(registerTok: Token): RegisterStatement | null {
    ...
    let changeDescription: StringLiteral | undefined;
  ...
  parseRegimeClause(
    ...
    const kwTok = this.expect(
      "keyword",
      keyword,
      "expected_token",
      `Expected ${keyword.toUpperCase()} clause`,
      `REGISTER boundary requires both BEFORE and AFTER clauses, each followed by a short label and a description string.`
    );
    ...
    if (!description) {
      ...
      this.errorAt(
        label.span,
        "expected_token",
        `${keyword.toUpperCase()} expects a short label string followed by a longer description string`
      );
      return null;
    }
