...
export function resolveConstraint(c: Constraint): ResolvedConstraint {
...
export function classifyRowAgainstBoundary(
  row: RowRegion,
  ...
  for (const c of boundary.constraints) {
    const rowVal = row.dimValues[c.dimension];
    if (rowVal === undefined) continue; // row aggregates this dim
    if (!constraintAllowsValue(c, rowVal)) return null;
  }
  if (row.timeEnd < boundary.at) return "before";
  if (row.timeStart >= boundary.at) return "after";
  return "straddles";
}
...
export function decorationsFor(
  ...
  return rows.map((row) => {
    ...
    let severity: "warn" | "error" | undefined;
    for (const m of matches) {
      if (m.kind === "region") {
        if (!rowMatchesImpact(rowRegion, m.impact.region)) continue;
        hits.push(m);
        ...
        hits.push({ ...m, side });
        if (side === "straddles") severity = "error";
