...
export async function complete(source, position, opts) {
    ...
    const consumed = allTokens.filter((t) => t.kind !== "eof" && t.span.end <= prefixSlice.start);
...
function findPrefix(source, pos) {
    let i = pos;
    while (i > 0 && isWordChar(source[i - 1]))
        i--;
...
function insideStringLiteral(source, pos) {
    let single = 0;
    let double = 0;
    for (let i = 0; i < pos; i++) {
        const ch = source[i];
...
function analyze(tokens) {
    ...
    if (headText === "compute")
        return analyzeCompute(tokens);
    if (headText === "register")
        return analyzeRegister(tokens);
    if (headText === "check")
        return analyzeCheck(tokens);
    if (headText === "show")
        return analyzeShow(tokens);
    if (headText === "unregister")
        return analyzeUnregister(tokens);
...
// ----- COMPUTE -----
function analyzeCompute(tokens) {
    // tokens[0] = COMPUTE
    if (tokens.length === 1)
        return { metric: true };
...
function analyzeQueryTail(tokens, startIdx, metric, options = { allowGroupingClauses: true }) {
    let i = startIdx;
    ...
    while (i < tokens.length && tokenIsKeyword(tokens[i], "and")) {
        ...
        if (next === null)
            return diagnoseConstraintGap(tokens, i, metric);
        i = next;
    }
    if (i >= tokens.length) {
        return tailAfterTimeRegion(metric, options);
...
function tailAfterTimeRegion(metric, options) {
    void metric;
    if (options.allowGroupingClauses) {
...
function analyzeGroupBy(tokens, startIdx, metric) {
    let i = startIdx;
    if (i >= tokens.length) {
...
function analyzeOrderBy(tokens, startIdx) {
    let i = startIdx;
    if (i >= tokens.length)
        return { identifier: true, grain: true };
    while (i < tokens.length) {
        if (tokens[i].kind !== "identifier" && tokens[i].kind !== "grain")
            break;
        i++;
        if (i >= tokens.length)
            return { keywords: ["ASC", "DESC", "LIMIT"] };
...
// ----- REGISTER -----
function analyzeRegister(tokens) {
    if (tokens.length === 1) {
    ...
    if (subject === "region")
        return analyzeRegisterRegion(tokens);
    if (subject === "boundary")
        return analyzeRegisterBoundary(tokens);
    return { softKeywords: ["region", "boundary"] };
}
function analyzeRegisterRegion(tokens) {
    // tokens[0]=REGISTER, [1]=region
    if (tokens.length === 2)
        return { identifier: true };
    // [2] = name
    if (tokens.length === 3)
        return { keywords: ["IMPACTING"] };
...
function analyzeRegisterBoundary(tokens) {
    ...
    while (i < tokens.length && tokenIsKeyword(tokens[i], "and")) {
        ...
        if (next === null)
            return diagnoseConstraintGap(tokens, i, undefined);
        i = next;
    }
    if (i >= tokens.length)
    ...
    if (!tokenIsKeyword(tokens[i], "after"))
        return { keywords: ["AFTER"] };
    i++;
    if (i >= tokens.length)
        return { stringLiteral: true };
...
// ----- UNREGISTER -----
function analyzeUnregister(tokens) {
    if (tokens.length === 1)
        return { lexiconEntry: true };
    return {};
}
...
function skipTimeRegion(tokens, startIdx) {
    let i = startIdx;
    if (i >= tokens.length)
        return null;
    const tok = tokens[i];
    ...
    if (tokenIsTimeKeyword(tok, "all")) {
        ...
        if (i >= tokens.length)
            return null;
        if (!tokenIsTimeKeyword(tokens[i], "time"))
            return null;
        return i + 1;
    }
...
function diagnoseTimeRegionGap(tokens, startIdx) {
...
function skipConstraint(tokens, startIdx, metric) {
    void metric;
    let i = startIdx;
    ...
    if (tokenIsKeyword(next, "not")) {
        i++;
        if (i >= tokens.length || !tokenIsKeyword(tokens[i], "in"))
            return null;
        i++;
...
function diagnoseConstraintGap(tokens, startIdx, metric) {
...
function skipParenSet(tokens, startIdx) {
    let i = startIdx;
    if (i >= tokens.length || tokens[i].kind !== "punctuation" || tokens[i].text !== "(") {
        return null;
    }
...
// ===== Materialization =====
async function materialize(expectation, opts) {
    const out = [];
...
function collectAllDimensions(adapter) {
    const seen = new Set();
    const out = [];
