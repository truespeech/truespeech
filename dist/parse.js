...
class Parser {
    ...
    parseStatement() {
        ...
        const tok = this.peek();
        this.errorAt(tok.span, "unexpected_token", `Expected COMPUTE, REGISTER, or CHECK, got "${tok.text}"`);
        return null;
    }
    // ===== COMPUTE =====
    parseCompute() {
    ...
    parseRegister() {
        ...
        // as a reserved word.
        if (!this.expect("identifier", "region", "expected_token", 'Expected entry kind "region" after REGISTER', "REGISTER takes an entry kind followed by the entry name; only `region` is currently defined")) {
            return null;
        }
