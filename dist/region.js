...
export function resolveRegion(over, primaryTimeField) {
...
function resolveConstraint(c) {
    ...
    switch (pred.kind) {
        ...
        case "in-set": {
            const values = pred.values.map(resolveValue);
...
export function renderTimeRegion(start, end) {
    if (start === end)
        return start;
    // Year alignment
...
export function renderRegion(region) {
    ...
    if (region.constraints.length === 0)
        return time;
...
export function formatTimeBucket(isoStart, grain) {
...
export function endOfBucket(isoStart, grain) {
...
function isFirstDayOfYear(iso) {
    return month(iso) === 1 && day(iso) === 1;
}
function isLastDayOfYear(iso) {
    return month(iso) === 12 && day(iso) === 31;
}
function isFirstDayOfQuarter(iso) {
    const m = month(iso);
    return day(iso) === 1 && (m === 1 || m === 4 || m === 7 || m === 10);
}
function isLastDayOfQuarter(iso) {
    const m = month(iso);
    return (day(iso) === daysInMonth(year(iso), m) &&
        (m === 3 || m === 6 || m === 9 || m === 12));
}
function isFirstDayOfMonth(iso) {
    return day(iso) === 1;
}
